...
def add_bg_from_url():
    """Adds a blurred background image and custom CSS for the container border."""
    st.markdown(
        """
        <style>
        .stApp {
        ...
        .stApp::before {
            ...
            right: 0;
            bottom: 0;
            background-image: url("https://static.vecteezy.com/system/resources/previews/026/275/002/large_2x/glass-of-beer-on-table-in-bar-or-pub-background-copy-space-for-print-beer-images-for-advertising-photo.jpeg");
...
# Call the function to set the background and container style
add_bg_from_url()
...
def load_and_train_model():
    """
    Loads the dataset, handles missing data, trains a Linear Regression model,
    and returns the trained model and features.
    """
    try:
    ...
    # The numeric_only=True parameter prevents a FutureWarning
    ...
    # Train the model on the full dataset for a more robust prediction
...
# --- Streamlit App UI ---
if model:
    st.title(' The Drink Predictor')
    st.markdown('Your personal alcohol consumption forecast.')
